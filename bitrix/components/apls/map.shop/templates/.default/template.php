...
<?
	...
	$arTransParams = array(
		...
		'ONMAPREADY' => 'APLS_SetPlacemarks_'.$arParams['MAP_ID'],
	);
...
function APLS_SetPlacemarks_<?echo $arParams['MAP_ID']?>(map)
{
	if(typeof window["APLS_YMapAddPlacemark"] != 'function')
	{
		...
		*/
		...
		var ymWaitIntervalId = setInterval( function(){
				if(typeof window["APLS_YMapAddPlacemark"] == 'function')
				{
                    APLS_SetPlacemarks_<?echo $arParams['MAP_ID']?>(map);
...
<?
	if (is_array($arResult['POSITION']['PLACEMARKS']) && ($cnt = count($arResult['POSITION']['PLACEMARKS']))):
		for($i = 0; $i < $cnt; $i++):
?>
    arObjects.PLACEMARKS[arObjects.PLACEMARKS.length] = APLS_YMapAddPlacemark(map,"<?=$templateFolder.'/images/label.svg'?>", <?echo CUtil::PhpToJsObject($arResult['POSITION']['PLACEMARKS'][$i])?><?if(count($arResult['POSITION']['PLACEMARKS'])>1):?>, true<?endif;?>);
//    arObjects.PLACEMARKS[arObjects.PLACEMARKS.length] = APLS_YMapAddPlacemark(map, <?//echo CUtil::PhpToJsObject($arResult['POSITION']['PLACEMARKS'][$i])?>//);
<?
		endfor;
	endif;
...
?>
    ...
    clusterer = new ymaps.Clusterer({
        ...
        clusterIcons: [
            {
                href: "<?=$templateFolder.'/images/circleLabel.svg'?>",
                size: [40, 40],
                offset: [-20, -20]
            },
//            {
