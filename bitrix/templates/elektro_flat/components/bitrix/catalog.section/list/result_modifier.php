...
foreach($arResult["ITEMS"] as $key => $arElement) {
	...
	if(isset($arPriceMatrix) && is_array($arPriceMatrix)) foreach($arPriceMatrix as $key_matrix => $item) {
		foreach($item as $key2 => $item2) {
			...
			if($inPriceRatio) {
				$arElement["PRICE_MATRIX"]["MATRIX"][$key_matrix][$key2]["DISCOUNT_PRICE"] = $arElement["CATALOG_MEASURE_RATIO"]*$arElement["PRICE_MATRIX"]["MATRIX"][$key_matrix][$key2]["DISCOUNT_PRICE"];
			}
		}
	}
					...
					if(true) {
						...
						foreach($arPriceMatrix as $key_matrix => $item) {
							foreach($item as $key2 => $item2) {
								...
								if($inPriceRatio) {
									$arOffer["PRICE_MATRIX"]["MATRIX"][$key_matrix][$key2]["DISCOUNT_PRICE"] = $arOffer["CATALOG_MEASURE_RATIO"]*$arOffer["PRICE_MATRIX"]["MATRIX"][$key_matrix][$key2]["DISCOUNT_PRICE"];
								}
							}
						}
					}
