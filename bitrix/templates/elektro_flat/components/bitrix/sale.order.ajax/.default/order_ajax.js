...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function(parameters)
		{
			...
			this.defCurrency = parameters.defCurrency || '';
			...
			this.defaultBasketItemLogo = this.templateFolder + "/images/logo.jpg";
			this.defaultStoreLogo = this.templateFolder + "/images/logo.jpg";
			this.defaultDeliveryLogo = this.templateFolder + "/images/logo.jpg";
			this.defaultPaySystemLogo = this.templateFolder + "/images/logo.jpg";
		...
		sendRequest: function(action, actionData)
		{
			...
			else if(this.personalDataErrors) {
				var alert = BX.create("DIV",{
					...
					children:[
						...
						BX.create("span",{
							...
							text: this.personalDataErrors
						}),
					]
				});
				
				var blockBad = BX.findChildren(BX("bx-soa-auth"),{className:"bx-soa-section-content reg"},true);
				var blockBadReg = BX.findChildren(BX(blockBad[0]),{className:"bx-soa-authform"},true);
				BX(blockBad[0]).insertBefore(alert,BX(blockBadReg[0]));
		...
		getErrorContainer: function(node)
		{
			...
			node.appendChild(
				BX.create('DIV', {props: {className: 'alertMsg bad'}, style: {display: 'none'}})
			);
		},
		...
		showError: function(node, msg, border)
		{
			...
			var errorContainer = node.querySelector('.alertMsg.bad'), animate;
			if (errorContainer && msg.length)
			{
				BX.cleanNode(errorContainer);
				errorContainer.appendChild(BX.create('I', {props: {className: 'fa fa-exclamation-triangle'}}));
				errorContainer.appendChild(BX.create('SPAN', {props: {className: 'text'}, html: msg}));
		...
		{
			var errorNodes = this.orderBlockNode.querySelectorAll('.alertMsg.bad'),
		...
		{
			var errorNode = node.querySelector('.alertMsg.bad'),
				hiddenNode, errors;
		...
		checkNotifications: function()
		{
			var informer = this.mainErrorsNode.querySelector('[data-type="informer"]'),
				success, sections, className, iconClassName, text, scrollTop, informerPos;
			...
			if (informer)
			{
				if (this.firstLoad && this.result.IS_AUTHORIZED && typeof this.result.LAST_ORDER_DATA.FAIL === 'undefined')
				{
					...
					className = success ? 'good' : 'bad';
					iconClassName = success ? 'check' : 'exclamation-triangle';
					...
					informer.appendChild(
						BX.create('I', {
							props: {className: 'fa fa-' + iconClassName}
						})
					);
					informer.appendChild(
						BX.create('SPAN', {
					...
					BX.addClass(informer, 'alertMsg ' + className);
					informer.style.display = '';
				}
				else if (BX.hasClass(informer, 'alertMsg'))
				{
		...
		{
			var hiddenNode, errorNode, showError, warningNode, showWarning, errorTooltips, i;

			if (hiddenNode = BX(node.id + '-hidden'))
			{
				errorNode = hiddenNode.querySelector('.alertMsg.bad');
				showError = errorNode && errorNode.style.display != 'none';
				
				warningNode = hiddenNode.querySelector('.alertMsg.info');
				showWarning = warningNode && warningNode.style.display != 'none';
		...
		scrollToError: function()
		{
			...
			for (i in sections)
			{
				...
				{
					errorNode = sections[i].querySelector('.alertMsg.bad');
		...
		showWarnings: function()
		{
			...
			if (currentDelivery && currentDelivery.CALCULATE_ERRORS)
			{
				...
				if (this.activeSectionId != this.deliveryBlockNode.id)
				{
					...
					BX.bind(this.deliveryBlockNode.querySelector('.alertMsg.info'), 'click', BX.proxy(this.showByClick, this));
				}
			}
		...
		showBlockWarning: function(node, warnings, hide)
		{
			...
			if (errorNode)
			{
				...
				warningNode = BX.create('DIV', {
					props: {className: 'alertMsg info'},
					children: [
						BX.create('I', {
							props: {className: 'fa fa-info'}
						}),
						BX.create('SPAN', {
							props: {className: 'text'},
							html: warnStr
		...
		bindEvents: function()
		{
			BX.bind(this.orderSaveBlockNode.querySelector('button'), 'click', BX.proxy(this.clickOrderSaveAction, this));
			...
			for (i in this.locations)
			{
				...
				if (clearButton)
				{
					BX.bind(clearButton, 'click', function(e){
						var target = e.target || e.srcElement,
							parent = BX.findParent(target, {tagName: 'DIV', className: 'bx-soa-form-group'}),
							locationInput;
		...
		clickOrderSaveAction: function(event)
		{
			if (this.isValidForm())
			{
				...
				if(BX("PERSONAL_DATA_order")) {
					if(BX("PERSONAL_DATA_order").value == "N"){
							...
							var alert = BX.create("span",{
								...
								children:[
									...
									BX.create("span",{
										...
										html:BX.message("NOT_FIELD_PERSONAL_DATA")
									})
								]
							});
							BX("hint_agreement").insertBefore(alert,BX("PERSONAL_DATA_order"));
						}
					} else {
		...
		fade: function(node, nextSection)
		{
			...
			switch (node.id)
			{
				...
				case this.regionBlockNode.id:
					...
					break;
				case this.paySystemBlockNode.id:
					BX.remove(this.paySystemBlockNode.querySelector('.alertMsg.info'));
					this.editFadePaySystemBlock();
					break;
				case this.deliveryBlockNode.id:
					BX.remove(this.deliveryBlockNode.querySelector('.alertMsg.info'));
		...
		editSection: function(section)
		{
			...
			errorContainer = section.querySelector('.alertMsg.bad');
		...
		editAuthorizeForm: function(authContent)
		{
			...
			remember = BX.create('DIV', {
				props: {className: 'bx-soa-authform-formgroup-container'},
				children: [
					BX.create('DIV', {
					...
					BX.create('INPUT', {
						...
						events: {
							click: BX.proxy(function(e){
								...
								if(BX("PERSONAL_DATA_order").value == "N"){
									this.personalDataErrors = BX.message("NOT_FIELD_PERSONAL_DATA");
								} else {
									BX.remove(BX("PERSONAL_DATA_BAD"));
									this.personalDataErrors = null;
								}
			...
			authFormNode = BX.create('DIV', {
				props: {className: 'bx-soa-authform'},
				children: [					
					login,
					password,
					remember,
		...
		createAuthFormInputContainer: function(labelText, inputNode, required)
		{
			...
			if (required)
				labelHtml += '<span class="bx-soa-authform-starrequired">*</span>';

			return BX.create('DIV', {
				props: {className: 'bx-soa-authform-formgroup-container'},
				children: [
					BX.create('DIV', {props: {className: 'bx-soa-authform-label-container'}, html: labelHtml}),
					BX.create('DIV', {props: {className: 'bx-soa-authform-input-container'},  children: [inputNode]})
				]
			});
		},
		...
		editRegistrationForm: function(authContent)
		{
			...
			if (this.result.AUTH.captcha_registration == 'Y')
			{
				authFormNodes.push(BX.create('DIV', {
					...
					children: [
						BX.create('DIV', {
							props: {className: 'bx-soa-authform-label-container'},
							children: [								
								BX.message('CAPTCHA_REGF_PROMT'),
								BX.create('SPAN', {props: {className: 'bx-soa-authform-starrequired'}, text: '*'}),
								BX.create('DIV', {
						...
						BX.create('DIV', {
							props: {className: 'bx-soa-authform-input-container'},
							children: [
								BX.create('INPUT', {
									attrs: {'data-send': true},
									props: {
			...
			if(this.params.SHOW_PERSONAL_DATA == 'Y') {
				authFormNodes.push(
					BX.create("DIV",{
						props:{
							className:"hint_agreement",
							id:"hint_agreement"
						},
						children:[
							BX.create("INPUT",{
							...
							BX.create("DIV",{
								...
								html:this.params.TEXT_PERSONAL_DATA
							}),
						]
					})
				);
		...
		editSocialContent: function(authContent)
		{
			...
			var nodes = [],
				socServiceHiddenNode = BX('bx-soa-soc-auth-services').querySelector('.bx-auth-serv-icons'),
				i;

			if (socServiceHiddenNode)
			{
				nodes.push(BX.create('DIV', {
						props: {className: 'bx-soa-soc-auth-services'},
						children: [
							BX.create('P', {text: BX.message('SOA_DO_SOC_SERV')}),
			...
			for(i = 0; i < nodes.length; i++) {
				authContent.appendChild(nodes[i]);
			}
		},
				...
				BX.create('DIV', {
					...
					children: [
						BX.create('DIV', {
							...
							children: [
								BX.create('DIV', {
									props: {className: 'bx-soa-section-title-count'},
									children: [
										BX.create('I', {
											props: {className: 'fa fa-check'}
										}),
										...
										BX.create('I', {
											props: {className: 'fa fa-exclamation-triangle'}
										})
									]
								}),
								BX.create('SPAN', {
		...
		editActiveBasketBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				basketContent.appendChild(basketTable);
		...
		editFadeBasketBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				newContent.appendChild(basketTable);
		...
		editBasketItemsHeader: function(basketItemsNode)
		{
			...
			var headers = [],
				column,				
				columnClass = 'bx-soa-item-td ',
				i;

			if(this.options.showPreviewPicInBasket || this.options.showDetailPicInBasket) {
				headers.push(
					BX.create('DIV', {
						props: {className: columnClass + 'bx-soa-item-image'},
						text: BX.message('SOA_SUM_IMAGE')
					})
				);
			}
			...
			headers.push(
				BX.create('DIV', {
					props: {className: columnClass + 'bx-soa-item-name'},
			...
			for (i = 0; i < this.result.GRID.HEADERS.length; i++)
			{
				...
				if (column.id == 'NAME' || column.id == 'PREVIEW_PICTURE' || column.id == 'DETAIL_PICTURE' || column.id == 'PROPS' || column.id == 'NOTES')
					continue;
				
				headers.push(
					BX.create('DIV', {
						props: {className: columnClass},
						text: column.name						
					})
				);
			}
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {className: 'bx-soa-item-thead'},
					children: headers
				})
			);
		},
		...
		createBasketItem: function(basketItemsNode, item, index, active)
		{
			...
			if(this.options.showPreviewPicInBasket || this.options.showDetailPicInBasket)
				allColumns.push(this.createBasketItemImg(item.data));

			allColumns.push(this.createBasketItemContent(item.data));

			for (i = 0; i < this.result.GRID.HEADERS.length; i++)
			{
				currentColumn = this.result.GRID.HEADERS[i];

				if (currentColumn.id == 'NAME' || currentColumn.id == 'PREVIEW_PICTURE' || currentColumn.id == 'DETAIL_PICTURE' || currentColumn.id == 'PROPS' || currentColumn.id == 'NOTES')
					continue;

				allColumns.push(this.createBasketItemColumn(currentColumn, item, active));
			}
		...
		createBasketItemImg: function(data)
		{
			...
			logoNode = BX.create('DIV', {props: {className: 'bx-soa-item-td bx-soa-item-image'}});
			...
			{
				logoNode.appendChild(BX.create('IMG', {
					props: {src: logotype.src_2x, alt: data.NAME}
				}));
			}
			else
			{
				...
				logoNode.appendChild(BX.create('IMG', {
					props: {src: logotype, alt: data.NAME}
				}));
			}
			...
			if (this.options.showPropsInBasket && props.length)
			{
				for (var i in props)
				{
					if (props.hasOwnProperty(i))
					{
						...
						propsHtml += '<br />' + name + ': ' + value;
					}
				}
			}

			return BX.create('DIV', {
				props: {className: 'bx-soa-item-td bx-soa-item-name'},
				children: propsHtml.length ? [
					titleHtml,
					propsHtml
				] : [
					titleHtml
		...
		createBasketItemColumn: function(column, allData, active)
		{
			...
			{				
				...
				{					
					textNode.push(BX.create('SPAN', {
						props: {className: 'bx-price-old'},
						html: data.BASE_PRICE_FORMATED
					}));
					textNode.push(BX.create('BR'));
				}				
				textNode.push(BX.create('SPAN', {props: {className: 'bx-price'}, html: data.PRICE_FORMATED}));

				if (this.options.showPriceNotesInBasket && active)
				{
					textNode.push(BX.create('BR'));
					textNode.push(BX.create('SMALL', {text: data.NOTES}));
				}
			}
			else if (column.id == 'SUM')
			{				
				textNode.push(BX.create('SPAN', {props: {className: 'bx-price'}, html: data.SUM}));
			}
			else if (column.id == 'DISCOUNT')
				textNode.push(data.DISCOUNT_PRICE_PERCENT_FORMATED);				
			else if (BX.util.in_array(column.id, ["QUANTITY", "WEIGHT_FORMATED", "DISCOUNT_PRICE_PERCENT_FORMATED"]))
				textNode.push(data[column.id]);				
			else
			{
				...
				if (BX.type.isArray(columnData))
				{
					for (var i in columnData)
					{
						...
						{
							if (columnData[i].type == 'linked')
							{
								textNode.push(columnData[i].value_format);
								textNode.push(BX.create('BR'));
							}
							else if (columnData[i].value)
							{
								textNode.push(columnData[i].value);
								textNode.push(BX.create('BR'));
							}
						}
					}
					...
					textNode.push(BX.util.htmlspecialchars(columnData));
			}

			return BX.create('DIV', {
				props: {className: 'bx-soa-item-td' + (!BX.util.in_array(column.id, ["QUANTITY", "SUM"]) ? ' hidden' : '')},
				children: textNode
			});
		},
		...
		popupShow: function(e, url, source)
		{
			...
			this.popup = new BX.PopupWindow('bx-soa-image-popup', null, {
				...
				content: BX.create('DIV', {
					props: {
						className: 'bx-soa-image-popup-content',
						id: 'bx-soa-image-popup-content'
					},
			...
			var close = BX.findChild(BX('bx-soa-image-popup'), {className: 'popup-window-close-icon'}, true, false);
			if(!!close)
		...
		editCouponsFade: function(basketItemsNode)
		{
			...
			{
				couponsLabel = this.getCouponsLabel();
				basketItemsNode.appendChild(
					BX.create('DIV', {
						props: {className: 'bx-soa-coupon-fade'},
		...
		getCouponNode: function(coupon, active)
		{
			...
			{
				case 'ENTERED': couponItem = 'disabled'; break;
				case 'BAD': couponItem = 'bad'; break;
				default: couponItem = 'good';
			}

			return BX.create(active ? 'DIV' : 'SPAN', {
				attrs: {
					'data-coupon': couponName,
					className: active ? 'bx-soa-coupon-item' : 'bx-soa-coupon-fade-item'
				},
				children: active ? [
					BX.create('DIV', {
						attrs: {							
							className: 'bx-soa-coupon-coupon ' + couponItem
						},
						html: couponName + ' ' + couponDesc
					}),
					BX.create('SPAN', {
						attrs: {					
							className: 'bx-soa-coupon-close'
						},
						children: [
							BX.create('I', {
								attrs: {
		...
		getCouponsLabel: function()
		{
			return BX.create('SPAN', {
				props: {className: 'bx-soa-coupon-fade-label'},
				children: [BX.util.htmlspecialchars(this.params.MESS_COUPON) + ': ']
			});
		},

		addCoupon: function(coupon)
		{
			var couponListNodes = this.orderBlockNode.querySelectorAll('.bx-soa-coupon .bx-soa-coupon-items'),
			...
			for (i = 0; i < couponListNodes.length; i++)
			{
				...
				couponListNodes[i].appendChild(this.getCouponNode({text: coupon}, true, 'bad'));
			}
		...
		editFadeRegionContent: function(node)
		{
			...
			var selectedPersonType = this.getSelectedPersonType(),
				errorNode = this.regionHiddenBlockNode.querySelector('.alertMsg.bad'),
			...
			BX.bind(node.querySelector('.alertMsg.bad'), 'click', BX.proxy(this.showByClick, this));
			BX.bind(node.querySelector('.alertMsg.info'), 'click', BX.proxy(this.showByClick, this));
		},
		...
		getDeliveryLocationInput: function(node)
		{
			...
			if (location && location[0] && location[0].output)
			{
				...
				labelHtml = '<label class="bx-soa-custom-label" for="soa-property-' + locationId + '">'					
					...
					+ (currentProperty.REQUIRED == 'Y' ? ' <span class="bx-soa-authform-starrequired">*</span>' : '')
				...
				insertedLoc = BX.create('DIV', {
					...
					props: {className: 'bx-soa-form-group bx-soa-location-input-container'},
			...
			if (altProperty)
			{
				...
				labelTextHtml += altProperty.REQUIRED == 'Y' ? ' <span class="bx-soa-authform-starrequired">*</span>' : '';
		...
		getZipLocationInput: function(node)
		{
			...
			if (zipProperty)
			{
				...
				propsItemNode = BX.create('DIV', {props: {className: "bx-soa-form-group bx-soa-location-input-container"}});
				propsItemNode.setAttribute('data-property-id-row', zipProperty.ID);
				
				labelTextHtml = BX.util.htmlspecialchars(zipProperty.NAME);
				labelTextHtml += zipProperty.REQUIRED == 'Y' ? ' <span class="bx-soa-authform-starrequired">*</span>' : '';
		...
		getPersonTypeControl: function(node)
		{
			...
			{
				input = BX.create('DIV', {
					props: {className: 'bx-soa-form-group'},
					children: [
			...
			else if (personTypesCount == 2)
			{
				for (i in this.result.PERSON_TYPE)
				{
					if (this.result.PERSON_TYPE.hasOwnProperty(i))
					{
						...
						node.appendChild(BX.create('DIV', {props: {className: 'bx-soa-radio-inline'}, children: [label]}));
		...
		createPaySystemItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				paySystemDescr,
				title, label, icon ,itemNode, timer;
			...
			{
				logoNode.appendChild(BX.create('IMG', {
					props: {src:logotype.src_2x, alt: item.PSA_NAME}
				}));
			}
			else
			{
				...
				logoNode.appendChild(BX.create('IMG', {
					props: {src: logotype, alt: item.PSA_NAME}
				}));
			}
			
			if(item.DESCRIPTION && item.DESCRIPTION.length) {
				if(item.PRICE && parseFloat(item.PRICE) > 0) {
					item.DESCRIPTION += '<br />' + this.params.MESS_PRICE + ': <span class="bx-soa-pp-company-price">~ ' + item.PRICE_FORMATTED + '</span>';
				}
				paySystemDescr = BX.create('DIV', {
					props: {className: 'bx-soa-pp-company-descr'},
					children: [
						BX.create('LABEL', {
							children: [
								BX.create('I', {
						...
						BX.create('DIV', {
							props: {className: 'pop-up pp-descr'},
			...
			label =  BX.create('DIV', {
				...
				children: [
					BX.create('DIV', {
						props: {className: 'bx-soa-pp-company-graf'},
						children: [
							BX.create('DIV', {
								...
								children: [
									BX.create('INPUT', {
										props: {
											...
											name: 'PAY_SYSTEM_ID',
											type: 'checkbox',
											className: 'bx-soa-pp-company-input',
											value: paySystemId,
											checked: checked
										}
									})
			...
			icon = BX.create('LABEL', {
				props: {className : "icon"},
				attr: {'for': 'ID_PAY_SYSTEM_ID_' + paySystemId},
				children: [
					BX.create('I', {
						props: {className: 'fa fa-check'}
					})
			...
			var popDesc = BX.findChild(BX(paySystemDescr),{className : "pop-up pp-descr"});
			
			BX.bind(BX(paySystemDescr),"mouseenter",function(){
				BX.adjust(BX(popDesc),{style : {opacity : 1,"z-index" : 1100}});
			});
			
			BX.bind(BX(itemNode),"mouseleave",function(){
				BX.adjust(BX(popDesc),{style : {opacity : 0,"z-index" : -1}});
			});
			
			return itemNode;
		},
		
		getInnerPaySystem: function(paySystemItemsContainer)
		{
			...
			var accountOnly = this.params.ONLY_FULL_PAY_FROM_ACCOUNT && (this.params.ONLY_FULL_PAY_FROM_ACCOUNT == 'Y'),
				...
				logotype, logoNode, paySystemDescr, currentBudget, label, title, hiddenInput, itemNode;			
			...
			{
				logoNode.appendChild(BX.create('IMG', {
					props: {src:logotype.src_2x, alt: paySystem.NAME}
				}));
			}
			else
			{
				logotype = logotype && logotype.src_1x || this.defaultPaySystemLogo;
				logoNode.appendChild(BX.create('IMG', {
					props: {src: logotype, alt: paySystem.NAME}
				}));
			}
			...
			currentBudget = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company-balance-wrap'},
				children: [
					BX.create('DIV', {
						props: {className: 'bx-soa-pp-company-balance'},
			...
			itemNode = BX.create('DIV', {
				...
				children: [
					BX.create('DIV', {
						...
						children: [hiddenInput, label, title],
						events: {
							click: BX.proxy(this.selectPaySystem, this)
						}
					})
		...
		editFadePaySystemContent: function(node)
		{
			var selectedPaySystem = this.getSelectedPaySystem(),
				errorNode = this.paySystemHiddenBlockNode.querySelector('.alertMsg.bad'),
				warningNode = this.paySystemHiddenBlockNode.querySelector('.alertMsg.info'),
			...
			if (this.isSelectedInnerPayment())
			{
				...
				addedHtml += '<div class="bx-soa-pp-company-selected-image"><img src="' + imgSrc + '" alt="" /></div>';
			...
			if (selectedPaySystem && selectedPaySystem.PSA_NAME)
			{
				...
				addedHtml += '<div class="bx-soa-pp-company-selected">';
				addedHtml += '<div class="bx-soa-pp-company-selected-image"><img src="' + imgSrc + '" alt="" /></div>';
				addedHtml += '<div class="bx-soa-pp-company-selected-title">' + selectedPaySystem.PSA_NAME + '</div>';
				addedHtml += '</div>';
			}
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			var currentDelivery, extraServices, extraServicesNode, period;
			...
			if (extraServices.length)
			{
				...
				deliveryNode.appendChild(extraServicesNode);
			}
			...
			if (currentDelivery.PERIOD_TEXT && currentDelivery.PERIOD_TEXT.length) {
				period = BX.create('DIV', {
					...
					children: [
						BX.create('DIV', {props: {className: 'bx-soa-pp-desc-period-termin'}, html: this.params.MESS_PERIOD}),
						BX.create('DIV', {props: {className: 'bx-soa-pp-desc-period-description'}, html: currentDelivery.PERIOD_TEXT})
					]
				});
				deliveryNode.appendChild(period);
			}
		},
			...
			for (i in delivery.EXTRA_SERVICES)
			{
				...
				if (currentService.editControl.indexOf('this.checked') == -1)
				{
					...
					serviceNode = BX.create('DIV', {
						...
						+ (currentService.description && currentService.description.length
							? '<div class="bx-soa-reference">' + BX.util.htmlspecialchars(currentService.description) + '</div>'
							: '')
					});
				...
				else
				{
					serviceNode = BX.create('DIV', {
						...
						children: [					
							BX.create('DIV', {
								props: {className: 'bx-soa-radio-inline'},
								children: [
									BX.create('LABEL', {
										children: [
		...
		createDeliveryItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				icon = BX.create('LABEL', {
					props: {className : "icon"},
					attr: {'for': 'ID_DELIVERY_ID_' + deliveryId},
					children: [
						BX.create('I', {
							props: {className: 'fa fa-check'}
						})
				...
				logotype, label, price, title, itemNode, logoNode;
			...
			{
				logoNode.appendChild(BX.create('IMG', {
					props: {src:logotype.src_2x, alt: this.params.SHOW_DELIVERY_PARENT_NAMES != 'N' ? item.NAME : item.OWN_NAME}
				}));
			}
			else
			{
				...
				logoNode.appendChild(BX.create('IMG', {
					props: {src: logotype, alt: this.params.SHOW_DELIVERY_PARENT_NAMES != 'N' ? item.NAME : item.OWN_NAME}
				}));
			}
			...
			if(item.DESCRIPTION && item.DESCRIPTION.length) {
				/*if(item.PERIOD_TEXT && item.PERIOD_TEXT.length) {
					item.DESCRIPTION += '<br />' + this.params.MESS_PERIOD + ': <span class="bx-soa-pp-company-period">' + item.PERIOD_TEXT + '</span>';
				}*/
			...
			if (item.PRICE >= 0 || typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined')
			{
				price = BX.create('DIV', {
					...
					html: typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined' ? item.DELIVERY_DISCOUNT_PRICE_FORMATED : item.PRICE_FORMATED
				});				
			}
			else if (deliveryCached && (deliveryCached.PRICE >= 0 || typeof deliveryCached.DELIVERY_DISCOUNT_PRICE !== 'undefined'))
			{
				price = BX.create('DIV', {
					props: {className: 'bx-soa-pp-delivery-cost'},
					html: typeof deliveryCached.DELIVERY_DISCOUNT_PRICE !== 'undefined' ? deliveryCached.DELIVERY_DISCOUNT_PRICE_FORMATED : deliveryCached.PRICE_FORMATED
				});
			}
			...
			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company'},
				children: [label, icon, price, title],
			...
			var popDesc = BX.findChild(labelNodes[2],{className : "pop-up pp-descr"});
	
			BX.bind(BX(labelNodes[2]),"mouseenter",function(){
				BX.adjust(BX(popDesc),{style : {opacity : 1,"z-index" : 1100}});
			});
		...
		editFadeDeliveryContent: function(node)
		{
			var selectedDelivery = this.getSelectedDelivery(),
				...
				errorNode = this.deliveryHiddenBlockNode.querySelector('.alertMsg.bad'),
				warningNode = this.deliveryHiddenBlockNode.querySelector('.alertMsg.info'),
		...
		activatePickUp: function(deliveryName)
		{
			...
			this.pickUpBlockNode.querySelector('.bx-soa-section-title').innerHTML =
				'<span class="bx-soa-section-title-count"><i class="fa fa-check" aria-hidden="true"></i><i class="fa fa-info" aria-hidden="true"></i><i class="fa fa-exclamation-triangle" aria-hidden="true"></i></span>' + deliveryName;
		...
		editActivePickUpBlock: function(activeNodeMode)
		{
			var node = activeNodeMode ? this.pickUpBlockNode : this.pickUpHiddenBlockNode,
				pickUpContent, pickUpContentNode;
			...
			else
			{
				...
				pickUpContentNode = BX.create('DIV', {props: {className: 'bx_soa_pickup'}});
				
				this.editPickUpMap(pickUpContentNode);
				pickUpContent.appendChild(pickUpContentNode);
		...
		editFadePickUpContent: function(pickUpContainer)
		{
			...
			if (selectedPickUp)
			{
				...
				html += '<div class="bx-soa-pp-company-selected-title">';
				...
				for (i = 0; i < activeStores.length; i++)
				{
					...
					geoObj = new ymaps.Placemark([activeStores[i].GPS_N, activeStores[i].GPS_S], {
						...
						balloonContentLayout: ymaps.templateLayoutFactory.createClass(
							'<div class="pickup-map-balloon-title">{{ properties.storeTitle }}</div>' +
							'{{ properties.storeBody|raw }}' +
							'<br /><button type="button" class="btn_buy popdef pickup-map-balloon-btn" data-store="{{ properties.id }}">{{ properties.text }}</button>',
							{
								build: function() {
									this.constructor.superclass.build.call(this);

									var button = document.querySelector('button[data-store]');
									if (button)
		...
		getStoreInfoHtml: function(currentStore)
		{
			...
			if (currentStore.ADDRESS)
				html += '<strong>' + BX.message('SOA_PICKUP_ADDRESS') + ':</strong> ' + BX.util.htmlspecialchars(currentStore.ADDRESS) + '<br />';

			if (currentStore.PHONE)
				html += '<strong>' + BX.message('SOA_PICKUP_PHONE') + ':</strong> ' + BX.util.htmlspecialchars(currentStore.PHONE) + '<br />';

			if (currentStore.SCHEDULE)
				html += '<strong>' + BX.message('SOA_PICKUP_WORK') + ':</strong> ' + BX.util.htmlspecialchars(currentStore.SCHEDULE) + '<br />';

			if (currentStore.DESCRIPTION)
				html += '<strong>' + BX.message('SOA_PICKUP_DESC') + ':</strong> ' + BX.util.htmlspecialchars(currentStore.DESCRIPTION) + '<br />';
		...
		createPickUpItem: function(currentStore, options)
		{
			...
			var logoNode, logotype, html, storeNode, imgSrc;
			...
			if (this.params.SHOW_STORES_IMAGES == 'Y')
			{
				...
				logoNode = BX.create('DIV', {
					props: {className: 'bx-soa-pickup-l-item-img-wrap'},
					children: [
						BX.create('IMG', {
							props: {
								src: imgSrc,
				...
				children: [
					BX.create('DIV', {
						...
						children: options.distance ? [
							BX.create('I', {
								props: {className: 'fa fa-map-marker'}
							}),
		...
		editFadePropsContent: function(node)
		{
			...
			var errorNode = this.propsHiddenBlockNode.querySelector('.alertMsg'),
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			{
				BX.addClass(propsItemNode, "bx-soa-form-group bx-soa-customer-field");
				
				textHtml += BX.util.htmlspecialchars(property.getName());

				if (property.isRequired())
					textHtml += ' <span class="bx-soa-authform-starrequired">*</span>';
			...
			if(!disabled && propertyDesc.length && propertyType != 'STRING' && propertyType != 'NUMBER' && propertyType != 'DATE') {
				propsItemNode.appendChild(BX.create('DIV', {
					props: {className: 'bx-soa-reference'},
					html: BX.util.htmlspecialchars(propertyDesc)
				}));
			}
		...
		insertLocationProperty: function(property, propsItemNode, disabled)
		{
			...
			if (property.getId() in this.locations)
			{
				...
				{
					propContainer = BX.create('DIV', {props: {className: 'bx-soa-location-input-container'}});
						...
						propContainer.appendChild(
							BX.create('INPUT', {
								attrs: {
									type: 'button',
									className: 'btn_buy popdef',
									'data-prop-id': property.getId(),
									value: BX.message('ADD_DEFAULT')
								},
								events: {
		...
		alterProperty: function(settings, propContainer)
		{
			...
			{
				BX.addClass(add[i], 'btn_buy popdef');
			...
			if (add.length)
			{
				...
				BX.bind(add, 'click', BX.proxy(function(e){
					...
					if (add.length && add[add.length - 2])
					{
						...
						BX.addClass(add[add.length - 2], 'btn_buy popdef');
					}
		...
		alterDateProperty: function(settings, inputText)
		{
			...
			addon = BX.create('DIV', {
				...
				children: [BX.create('I', {props: {className: 'fa fa-calendar'}})]
			});
			...
			if (propsErrors.length && !navigated)
			{
				if (this.activeSectionId == this.propsBlockNode.id)
				{
					...
					for (i = 0; i < tooltips.length; i++)
					{
						...
						{
							this.animateScrollTo(BX.findParent(tooltips[i], {className: 'bx-soa-form-group bx-soa-customer-field'}), 800, 50);
							break;
						}
					}
				}
		...
		showErrorTooltip: function(tooltipId, targetNode, text)
		{
			...
			if (tooltip && tooltip.querySelector('span.text'))
			{
				tooltipInner = tooltip.querySelector('span.text');
			}
			else
			{
				tooltipInner = BX.create('SPAN', {props: {className: 'text'}});
				tooltip = BX.create('DIV', {
					props: {
						id: 'tooltip-' + tooltipId,
						className: 'tooltip alertMsg bad'
					},
					children: [
						BX.create('I', {props: {className: 'fa fa-exclamation-triangle'}}),
						tooltipInner
					]
				});
		...
		showValidationResult: function(inputs, errors)
		{
			...
			var input0 = BX.type.isElementNode(inputs[0]) ? inputs[0] : inputs[0][0],
				formGroup = BX.findParent(input0, {tagName: 'DIV', className: 'bx-soa-form-group'}),
			...
			for (i = 0; i < inputs.length; i++)
			{
				inputDiv = BX.findParent(inputs[i], {tagName: 'DIV', className: 'bx-soa-form-group'});
		...
		validateLocation: function(input, arProperty, fieldName)
		{
			...
			var parent = BX.findParent(input, {tagName: 'DIV', className: 'bx-soa-form-group'}),
		...
		{
			var propsMapContainer = BX.create('DIV', {props: {className: 'bx-soa-form-group'}}),
		...
		editPropsComment: function(propsNode)
		{
			...
			div = BX.create('DIV', {
				props: {className: 'bx-soa-form-group bx-soa-customer-field'},
		...
		editTotalBlock: function()
		{
			...
			if (this.options.showPayedFromInnerBudget)
			{
				...
				if(this.params.REFERENCE_PRICE == 'Y' && !!this.params.REFERENCE_PRICE_COEF) {
					this.totalInfoBlockNode.appendChild(
						BX.create('DIV', {
							props: {className: 'bx-soa-cart-total-line'},
							children: [
								BX.create('SPAN', {
									props: {
										className: 'bx-soa-cart-d'
									},
									html: BX.Currency.currencyFormat(total.ORDER_TOTAL_LEFT_TO_PAY * this.params.REFERENCE_PRICE_COEF, this.defCurrency, true)									
								})
							]
						})
					);
						...
						BX.create('DIV', {
							...
							children: [
								BX.create('SPAN', {
									...
									html: BX.Currency.currencyFormat(total.ORDER_TOTAL_PRICE * this.params.REFERENCE_PRICE_COEF, this.defCurrency, true)									
								})
							]
						})
					);
			...
			if (!this.result.SHOW_AUTH)
			{
				this.totalInfoBlockNode.appendChild(
					BX.create('DIV', {
						props: {className: 'bx-soa-cart-total-button-container' + (!showOrderButton ? ' hidden' : '')},
						children: [
							BX.create('BUTTON', {
								props: {
									type: 'button',
		...
		{
			if (this.result.SHOW_AUTH)
				BX.removeClass(this.mobileTotalBlockNode, 'hidden');
			else
				BX.addClass(this.mobileTotalBlockNode, 'hidden');
			...
			BX.bind(this.mobileTotalBlockNode.querySelector('button'), 'click', BX.proxy(this.clickOrderSaveAction, this));
		},
