		...
		if($haveOffers) {
			if($arElement["TOTAL_OFFERS"]["MIN_PRICE"]["RATIO_PRICE"] <= 0) {?>							
				<div class="item-no-price">			
					<span class="unit">
						<?=Loc::getMessage("CT_BCS_BIGDATA_ELEMENT_NO_PRICE")?>
		...
		if($haveOffers) {
			...
			<div class="available">
				<?if($arElement["TOTAL_OFFERS"]["QUANTITY"] > 0 || !$arElement["CHECK_QUANTITY"]) {?>	
					<div class="avl">
						...
						<span>
							<?=Loc::getMessage("CT_BCS_BIGDATA_ELEMENT_AVAILABLE");
			...
			<div class="add2basket_block">
				<form action="javascript:void(0)" class="add2basket_form">
					...
					<button type="button" id="<?=$itemIds['PROPS_BTN']?>" class="btn_buy" name="add2basket"><i class="fa fa-shopping-cart"></i><span><?=Loc::getMessage("CT_BCS_BIGDATA_ELEMENT_ADD_TO_CART")?></span></button>
				</form>
			</div>
				...
				<?if($arElement["CAN_BUY"]) {
					...
					<?} else {
						...
						<?}?>
							...
							<button type="button" id="<?=(isset($arElement['SELECT_PROPS']) && !empty($arElement['SELECT_PROPS']) ? $itemIds['PROPS_BTN'] : $itemIds['BTN_BUY']);?>" class="btn_buy" name="add2basket"><i class="fa fa-shopping-cart"></i><span><?=Loc::getMessage("CT_BCS_BIGDATA_ELEMENT_ADD_TO_CART")?></span></button>
						</form>
					<?}
				} elseif(!$arElement["CAN_BUY"]) {
					//ITEM_UNDER_ORDER//?>
