...
$arResult["ELEMENT"]['PRICE_MATRIX'] = CatalogGetPriceTableEx($arResult["ELEMENT"]['ID'], 0, $arPriceTypeID, 'Y');
...
if(count($arResult["ELEMENT"]['PRICE_MATRIX']["COLS"]) > 1) {
	...
	$arResult["ELEMENT"]["PRICE_PRINT_VALUE"] = number_format($arResult["ELEMENT"]["PRICE_VALUE"], $price["DECIMALS"], $price["DEC_POINT"], $price["THOUSANDS_SEP"]).$currency;
	$arResult["ELEMENT"]["PRICE_PRINT_DISCOUNT_VALUE"] = number_format($arResult["ELEMENT"]["PRICE_DISCOUNT_VALUE"], $price["DECIMALS"], $price["DEC_POINT"], $price["THOUSANDS_SEP"]).$currency;
}

if($inPriceRatio) {
	$arResult["ELEMENT"]["PRICE_DISCOUNT_VALUE"] = min($discountPrice)*$arResult["ELEMENT"]["MEASURE_RATIO"];
	$arResult["ELEMENT"]["PRICE_VALUE"] = min($price)*$arResult["ELEMENT"]["MEASURE_RATIO"];
	$arResult["ELEMENT"]["PRICE_PRINT_VALUE"] = number_format($arResult["ELEMENT"]["PRICE_VALUE"], $price["DECIMALS"], $price["DEC_POINT"], $price["THOUSANDS_SEP"]).$currency;
	$arResult["ELEMENT"]["PRICE_PRINT_DISCOUNT_VALUE"] = number_format($arResult["ELEMENT"]["PRICE_DISCOUNT_VALUE"], $price["DECIMALS"], $price["DEC_POINT"], $price["THOUSANDS_SEP"]).$currency;
}
...
foreach($arResult["SET_ITEMS"]["DEFAULT"] as $key => $arItem) {
	...
	$arItem['PRICE_MATRIX'] = CatalogGetPriceTableEx($arItem['ID'], 0, $arPriceTypeID, 'Y');
