									...
									foreach($arResult["OFFERS"] as $key => $arOffer) {?>
										<div id="detail_price_<?=$arItemIDs['ID'].'_'.$arOffer['ID']?>" class="detail_price<?=($key == $arResult['OFFERS_SELECTED'] ? '' : ' hidden');?>">											
											...
											<?} else {
                                                ...
                                                <?=APLS_CatalogItemInfo::getRegisterPrice($arResult["ID"],$arResult["PRICES"])?>
												...
												<?} else {?>
													<div class="add2basket_block">
														<form action="<?=SITE_DIR?>ajax/add2basket.php" class="add2basket_form">
															...
															<?}?>
															<button type="button" class="btn_buy detail" name="add2basket"><i class="fa fa-shopping-cart"></i><span><?=GetMessage('CATALOG_ELEMENT_ADD_TO_CART')?></span></button>
														</form>
...
<!--				if(!empty($arResult["DISPLAY_PROPERTIES"]) || !empty($sPropOffers)) {?>-->
									...
									if($arOffer["CAN_BUY"]) {
										...
										<?} else {
											...
											<div class="add2basket_block">
												<?//OFFERS_LIST_DELAY//?>
												<div class="delay">
													<a href="javascript:void(0)" id="catalog-item-delay-<?=$arItemIDs['ID'].'-'.$arOffer['ID']?>" class="catalog-item-delay" onclick="return addToDelay('<?=$arOffer["ID"]?>', 'quantity_<?=$arItemIDs["ID"]."_".$arOffer["ID"]?>', '<?=$props?>', '', 'catalog-item-delay-<?=$arItemIDs["ID"]."-".$arOffer["ID"]?>', '<?=SITE_DIR?>')" rel="nofollow"><i class="fa fa-archive"></i><i class="fa fa-check"></i></a>
												</div>
	...
	<div class="tabs-wrap tabs-catalog-detail">
        <?$ServiceCentersText = APLS_CatalogItemInfo::getServiceCenters($arResult["PROPERTIES"]["Servis"]["VALUE"]);?>
		<ul class="tabs">
			<?$i = 1;?>
            ...
            if(!empty($arResult["DISPLAY_PROPERTIES"]) || !empty($sPropOffers)) {?>
                <li class="tabs__tab">
                    <a href="#tab<?=$i?>"><span><?=GetMessage("CATALOG_ELEMENT_PROPERTIES")?></span></a>
                </li>
                <?$i++;
            }
            if(!empty($arResult["PROPERTIES"]["Servis"]["VALUE"]) && trim($ServiceCentersText) != "") {?>
                <li class="tabs__tab">
                    <a href="#tab<?=$i?>"><span><?=GetMessage("CATALOG_ELEMENT_SERVICE_CENTERS")?></span></a>
                </li>
                <?$i++;
            }
        ...
        if(!empty($arResult["DISPLAY_PROPERTIES"]) || !empty($sPropOffers)) :?>
            <div class="tabs__box">
                <div class="tabs__box-content">
                    <?if(!empty($arResult["DISPLAY_PROPERTIES"]) || !empty($sPropOffers)) {?>
                        <table class="apls-product-details-table">
                        <?//DETAIL_PROPERTIES//
                        if(!empty($arResult["DISPLAY_PROPERTIES"])) {
                            foreach($arResult["DISPLAY_PROPERTIES"] as $k => $v) {?>
                                <tr class="apls-catalog-detail-property">
                                    <td class="name">
