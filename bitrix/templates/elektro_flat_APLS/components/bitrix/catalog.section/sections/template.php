				...
				<div class="catalog-item-cards">
					...
					foreach($arSection["ITEMS"] as $key => $arElement) {
						...
						if(array_key_exists("PROPERTIES", $arElement) && is_array($arElement["PROPERTIES"])) {
                            ...
                            $sticker .= APLS_CatalogItemInfo::getLables($arElement['ID'],$arElement['XML_ID'],$arElement["PROPERTIES"]);
							//NEW//
										...
										<div class="catalog-price-ranges">
											<?foreach($arElement["TOTAL_OFFERS"]["PRICE_MATRIX_SHOW"]["COLS"] as $key_matrix => $item) {
												...
												<div class="catalog-detail-price-ranges__row">
													...
													<?if($countRange > 1):?>
														<span class="catalog-item-price-ranges-wrap">
															<a id="<?=$arElement["TOTAL_OFFERS"]["ID_PRICE_MATRIX_BTN"][$key_matrix]?>" data-key="<?=$key_matrix?>" class="catalog-item-price-ranges" href="javascript:void(0);">
										...
										<div class="catalog-price-ranges">
											<?foreach($arElement["PRICE_MATRIX_SHOW"]["COLS"] as $key_matrix => $item) {
												...
												<div class="catalog-detail-price-ranges__row">
													...
													<?if($countRange > 1):?>
														<span class="catalog-item-price-ranges-wrap">
															<a id="<?=$arElement["ID_PRICE_MATRIX_BTN"][$key_matrix]?>" data-key="<?=$key_matrix?>" class="catalog-item-price-ranges" href="javascript:void(0);">
																<i class="fa fa-question-circle-o"></i>
															</a>
														</span>
													<?endif;?>
								...
								<div class="buy_more">
									...
									//ITEM_DELAY//
										...
										if($arElement["CAN_BUY"] && $arElement["MIN_PRICE"]["RATIO_PRICE"] > 0) {
											...
											<div class="delay">
												<a href="javascript:void(0)" id="catalog-item-delay-<?=$arItemIDs['ID']?>" class="catalog-item-delay" onclick="return addToDelay('<?=$arElement["ID"]?>', 'quantity_<?=$arItemIDs["ID"]?>', '<?=$props?>', '', 'catalog-item-delay-<?=$arItemIDs["ID"]?>', '<?=SITE_DIR?>')" title="<?=GetMessage('CATALOG_ELEMENT_ADD_TO_DELAY')?>" rel="nofollow"><i class="fa fa-archive"></i><i class="fa fa-check"></i></a>
											</div>
										<?}
									}?>								
								</div>
...
<script type="text/javascript">
	BX.ready(function() {
		BX.message({			
			SECTIONS_ELEMENT_FROM: "<?=GetMessageJS('CATALOG_ELEMENT_FROM')?>",
			...
			if((isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])) || $arElement["SELECT_PROPS"]) {				
				$arJSParams = array(					
					"VISUAL" => array(
						...
						"PRICE_MATRIX_BTN_ID" => is_array($arElement["TOTAL_OFFERS"]["ID_PRICE_MATRIX_BTN"]) ? $arElement["TOTAL_OFFERS"]["ID_PRICE_MATRIX_BTN"] : ""
					),
					"PRODUCT" => array(
						"ID" => $arElement["ID"],
