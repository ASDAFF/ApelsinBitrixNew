...
foreach($arResult["ITEMS"] as $key => $arElement) {
	...
	foreach($arPriceMatrix as $key_matrix => $item) {
		foreach($item as $key2 => $item2) {
			...
			//BTN_PRICE_MATRIX//
			$arResult["ITEMS"][$key]["ID_PRICE_MATRIX_BTN"][$key_matrix] = $arResult["ITEMS"][$key]["STR_MAIN_ID"]."_price_ranges_btn"."_".$key_matrix;
		}
	}
					...
					if(true) {
						...
						foreach($arPriceMatrix as $key_matrix => $item) {
							foreach($item as $key2 => $item2) {
								...
								//BTN_PRICE_MATRIX//
								$arResult["ITEMS"][$key]["TOTAL_OFFERS"]["ID_PRICE_MATRIX_BTN"][$key_matrix] = $arResult["ITEMS"][$key]["STR_MAIN_ID"]."_price_ranges_btn"."_".$key_matrix;
							}
						}
					}
