	...
	<form method="post"  class="sale-profile-detail-form" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
        ...
        foreach ($propVals as $props) {
            foreach ($props as $key=>$prop) {
                if ($key == '2' || $key == '3') {
                    $resultArray[$key] = $prop['VALUE'];
                }
            }
        }
        ?>
        <pre><?var_dump($resultArray);?></pre>
		<input type="hidden" name="ID" value="<?=$profId?>">
        <?
//        $arResult["ORDER_PROPS"][1]["PROPS"][0]["REQUIED"] = 'N';
//        $arResult["ORDER_PROPS"][1]["PROPS"][0]["~REQUIED"] = 'N';
        $arResult["ORDER_PROPS"][1]["PROPS"][1]["REQUIED"] = 'N';
//        $arResult["ORDER_PROPS"][1]["PROPS"][1]["~REQUIED"] = 'N';
        $arResult["ORDER_PROPS"][1]["PROPS"][2]["REQUIED"] = 'N';
//        $arResult["ORDER_PROPS"][1]["PROPS"][2]["~REQUIED"] = 'N';
//        $arResult["ORDER_PROPS"][2]["PROPS"][0]["REQUIED"] = 'N';
//        $arResult["ORDER_PROPS"][2]["PROPS"][0]["~REQUIED"] = 'N';
//        $arResult["ORDER_PROPS"][2]["PROPS"][1]["REQUIED"] = 'N';
//        $arResult["ORDER_PROPS"][2]["PROPS"][1]["~REQUIED"] = 'N';
//        $arResult["ORDER_PROPS"][2]["PROPS"][3]["REQUIED"] = 'N';
//        $arResult["ORDER_PROPS"][2]["PROPS"][3]["~REQUIED"] = 'N';
        ?>
        <?array_pop($arResult["ORDER_PROPS"]);?>
		<?foreach($arResult["ORDER_PROPS"] as $block) {
			if(!empty($block["PROPS"])) {
			    array_shift($block["PROPS"])?>
